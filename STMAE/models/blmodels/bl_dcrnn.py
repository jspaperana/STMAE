...
from .bl_utils import calculate_scaled_laplacian, calculate_random_walk_matrix
...
class EncoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, adj_mx, device=torch.device('cpu'), **model_kwargs):
...
class DCRNN_BL(nn.Module, Seq2SeqAttrs):
    def __init__(self, adj_mx, device=torch.device('cpu'), **model_kwargs):
        super().__init__()
        Seq2SeqAttrs.__init__(self, adj_mx, **model_kwargs)
    ...
    def decoder(self, encoder_hidden_state, labels=None, batches_seen=None):
        ...
        :return: output: (self.horizon, batch_siz-e, self.num_nodes * self.output_dim)
        """
    ...
    def forward(self, inputs, labels=None, batches_seen=None, *args, **kwargs):
        """
        ...
        """
        # assume input/output shape: (B, T, N, D)
        N = inputs.size(2)
        inputs = rearrange(inputs, 'b t n d -> t b (n d)')
        encoder_hidden_state = self.encoder(inputs)
        if labels is not None:
            labels = rearrange(labels, 'b t n d -> t b (n d)')
        outputs = self.decoder(encoder_hidden_state, labels, batches_seen=batches_seen)
        outputs = rearrange(outputs, 't b (n d) -> b t n d', n=N)
        if batches_seen == 0:
            print(">>> STGCL baseline trainer params: {:.2f}M".format(sum(p.numel() for p in self.parameters() if p.requires_grad) / 1000000.0))
        return outputs